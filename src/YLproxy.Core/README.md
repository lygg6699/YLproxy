...
- 自动创建不存在的目录和文件
- 使用 Windows DPAPI 保护用户名和密码，加载时仅在内存中解密
- 自动识别旧明文配置并迁移，使用原子替换降低写入中断损坏风险

### Config/ProxyDataSerializer.cs
代理数据存储转换：
- 负责内存模型与加密 JSON 存储模型之间的转换
- 使用 `dpapi:v1:` 前缀识别新密文，旧明文通过加载保存流程迁移
