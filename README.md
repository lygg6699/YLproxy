...
YLproxy/
├── AppSettings.json                 # 应用程序全局配置（日志、代理范围、主题等）
├── YLproxy.sln / YLproxy.slnx      # 解决方案文件
├── .agent.md                        # 开发代理行为指令（供 AI agent 使用）
│
├── src/                             # —————— 源代码 ——————
│   ├── YLproxy.Models/              # 数据模型层：ProxyItem、配置结构体等数据类
│   ├── YLproxy.Utils/               # 工具函数库（预留，当前为空）
│   ├── YLproxy.Core/                # 核心业务逻辑层
│   │   ├── ConfigService.cs         #   config.json 加载/保存
│   │   ├── ProxyTester.cs           #   代理连通性测试（HTTP 请求 + 延迟测量）
│   │   └── MonitorService.cs        #   后台 5 秒轮询监控代理进程状态
...
│       └── AddProxyWindow/          #   添加代理弹窗（表单校验、端口分配）
│
├── runtime/                         # —————— 运行时依赖 ——————
│   └── 3proxy/                      # 3proxy 代理引擎可执行文件及插件
│       ├── bin64/3proxy.exe         #   64位 3proxy 主程序
│       ├── FilePlugin.dll           #   文件插件
│       └── StringsPlugin.dll        #   字符串处理插件
│
├── data/                            # —————— 数据目录（运行时生成） ——————
│   └── config.json                  #   代理配置持久化 JSON
│
├── logs/                            # —————— 日志目录（运行时生成） ——————
│
├── docs/                            # —————— 项目文档 ——————
...
json
{
  ...
  "Application": {
    ...
    "StartupMinimized": false     // 是否启动时最小化到托盘
  },
  "ThreeProxy": {
